...
MAP_LEVEL = 'Normal' # ['Warmup', 'Normal', 'Complex', 'Extrem']
NAVIGATION_DIFFICULTY_LEVELS = ('Warmup', 'Normal', 'Complex', 'Extrem')
...
BLOCK_MIXING_PLANT_CONFIG = {
    "Warmup": {
        ...
        "dead_end_count_range": (0, 0),

        "segment_length_range": (0, 0),
        "turn_probability": 0.0,

        "parking_bay_count_range": (1, 1),
        "min_parking_bay_count": 1,
        ...
        "scene_metric_max_path_ratio": 1.75,
        "scene_metric_min_path_clearance": 0.90,
        "scene_metric_min_endpoint_clearance": 0.70,
        "scene_metric_tight_turn_heading_deg": 100.0,
        "scene_metric_tight_turn_min_endpoint_clearance": 0.80,

        "block_rotation_deg_max": 0.0,
        "block_position_jitter_ratio": 0.0,

        "boundary_margin": 2,
        "max_generation_attempts": 120,
        "max_parking_bay_attempts": 32,
        "max_pose_sample_attempts": 1,
        "obstacle_merge_mode": "rect_merge",
        "seed": None,

        "warmup_bay_margin_cells": 12,
        "warmup_path_buffer_m": 2.0,
        "warmup_motion_primitive_count_range": (20, 40),
        "warmup_motion_candidate_trials_per_step": 10,
        "warmup_motion_max_stall_steps": 8,
    },
...
SOFT_MASK_SAFETY_MARGIN = 0.0
SOFT_MASK_REVERSE_MARGIN_SCALE = 1.0

# PPO regular macro-action execution can shorten primitive prefixes in narrow scenes
# even without takeover. This only affects wrapper-side execution cadence, not the
# policy inter.
PPO_DYNAMIC_PREFIX_ENABLE = True
PPO_DYNAMIC_PREFIX_MIN_STEPS = 1
PPO_DYNAMIC_PREFIX_NARROW_STEPS = 1
PPO_DYNAMIC_PREFIX_MAX_STEPS = None
PPO_DYNAMIC_PREFIX_NARROW_MIN_LIDAR = 0.1
PPO_DYNAMIC_PREFIX_CRITICAL_MIN_LIDAR = 1.5
PPO_DYNAMIC_PREFIX_DENSE_OBS_LOW = 0.20
PPO_DYNAMIC_PREFIX_DENSE_OBS_HIGH = 0.35
PPO_DYNAMIC_PREFIX_LOW_ACTION_COUNT = 4
PPO_DYNAMIC_PREFIX_CRITICAL_ACTION_COUNT = 2

# Safety-prefix and blocked-transition semantics for PPO + primitive execution
MIN_SAFE_PREFIX_STEPS = 1
BLOCKED_ACTION_REWARD = -0.1
MAX_CONSECUTIVE_BLOCKED_ACTIONS = 35
...
PRIMITIVE_MODE_SELECTOR_HIGH = 0.3
PRIMITIVE_MODE_SELECTOR_LOW = 0.15
PRIMITIVE_MODE_HYSTERESIS_STEPS = 3
PRIMITIVE_MODE_PROGRESS_WINDOW = 6
PRIMITIVE_MODE_STUCK_STEPS = 5
PRIMITIVE_MODE_CLEARANCE_SAFE = 2.5
...
# -----------------------------
# Training-time primitive-lattice Hybrid A* guidance sidecar.
# Guidance is advisory only and keeps PPO as the sampler/executor.
# -----------------------------
ENABLE_HYBRID_ASTAR_GUIDANCE = True
HYBRID_ASTAR_GUIDANCE_CURRICULUM_ENABLE = {
    'Warmup': True,
    'Normal': True,
    'Complex': False,
    'Extrem': False,
}
HYBRID_ASTAR_GUIDANCE_LAMBDA_BY_CURRICULUM = {
    'Warmup': 1.25,
    'Normal': 0.60,
    'Complex': 0.05,
    'Extrem': 0.0,
}
HYBRID_ASTAR_GUIDANCE_DROPOUT_BY_CURRICULUM = {
    'Warmup': 0.00,
    'Normal': 0.25,
    'Complex': 0.75,
    'Extrem': 1.00,
}
HYBRID_ASTAR_GUIDANCE_REPLAN_EVERY_BY_CURRICULUM = {
    'Warmup': 1,
    'Normal': 2,
    'Complex': 4,
    'Extrem': 999999,
...
HYBRID_ASTAR_GUIDANCE_LOCAL_SUBGOAL_HORIZON_BY_CURRICULUM = {
    'Warmup': 4,
    'Normal': 4,
    'Complex': 3,
    'Extrem': 2,
...
HYBRID_ASTAR_GUIDANCE_MAX_EXPAND_NODES = 256
HYBRID_ASTAR_GUIDANCE_MAX_PLAN_TIME_MS = 80.0
HYBRID_ASTAR_GUIDANCE_MAX_SEARCH_DEPTH = 5
HYBRID_ASTAR_GUIDANCE_HEURISTIC_WEIGHT = 1.35
HYBRID_ASTAR_GUIDANCE_GREEDY_FALLBACK_ENABLE = True
HYBRID_ASTAR_GUIDANCE_POSITION_RESOLUTION_M = 0.75
HYBRID_ASTAR_GUIDANCE_HEADING_RESOLUTION_DEG = 12.0
HYBRID_ASTAR_GUIDANCE_GAMMA_RESOLUTION_DEG = 6.0
HYBRID_ASTAR_GUIDANCE_COLLISION_MAX_TRANSLATION_M = 0.55
HYBRID_ASTAR_GUIDANCE_COLLISION_MAX_HEADING_DEG = 8.0
...
HYBRID_ASTAR_GUIDANCE_JACKKNIFE_MARGIN_WARN_DEG = 4.0
HYBRID_ASTAR_GUIDANCE_REAR_NEAR_COLLISION_MARGIN_M = 0.35
HYBRID_ASTAR_GUIDANCE_REVERSE_PENALTY = 0.25
HYBRID_ASTAR_GUIDANCE_DIRECTION_SWITCH_PENALTY = 0.20
...
HYBRID_ASTAR_GUIDANCE_FINAL_FRONT_OVERLAP_THR = 0.70
HYBRID_ASTAR_GUIDANCE_FINAL_HEADING_TOL_DEG = 15.0
HYBRID_ASTAR_GUIDANCE_TEACHER_LOGIT = 2.5
HYBRID_ASTAR_GUIDANCE_NEIGHBOR_LOGIT = 0.75
HYBRID_ASTAR_GUIDANCE_CACHE_SIZE = 2048
HYBRID_ASTAR_GUIDANCE_FAIL_DISABLE_STREAK = 3
HYBRID_ASTAR_GUIDANCE_APPLY_IN_EVAL = False
HYBRID_ASTAR_GUIDANCE_EVAL_EVERY = 0
HYBRID_ASTAR_GUIDANCE_BC_ENABLE = False
HYBRID_ASTAR_GUIDANCE_BC_WEIGHT_BY_CURRICULUM = {
    'Warmup': 0.05,
    'Normal': 0.01,
    'Complex': 0.0,
    'Extrem': 0.0,
}
HYBRID_ASTAR_GUIDANCE_BC_MASK_THRESHOLD = 0.05
HYBRID_ASTAR_GUIDANCE_BC_CONFIDENCE_THRESHOLD = 0.10
...
AP_W_UTILITY = 0.40
AP_W_NOVELTY = 0.25
AP_UTILITY_HARD_RECOVERY_BONUS = 0.10

# Add / cap
AP_MAX_ADD_PER_ROUND = 12
AP_MAX_LIBRARY_SIZE = PRIMITIVE_GAMMA_BINS * PRIMITIVE_FAMILY_ACTION_DIM * PRIMITIVE_ADAPTIVE_MAX_VARIANTS_PER_FAMILY
