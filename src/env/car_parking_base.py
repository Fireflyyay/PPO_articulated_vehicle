...
class CarParking(gym.Env):
    ...
    def reset(self, seed: int = None, options: dict = None) -> tuple:
        ...
        terminal_metrics = self._terminal_metrics()
        info = {
    ...
    def _terminal_metrics(self) -> dict:
        try:
            ...
            heading_error = float(self._get_angle_diff(float(state.heading), float(dest.heading)))
            return {
                'dist_to_dest': float(state.loc.distance(dest.loc)),
    ...
    def get_reward(self, action, prev_state: State = None):
        ...
        for box_idx, box in enumerate(vehicle_boxes):
            ...
            for obst in self.map.obstacles:
                if box_polygon.intersects(obst.shape):
                    is_collision = True
                    collision_parts.append('front' if int(box_idx) == 0 else 'rear')
                    break
        ...
        if status == Status.CONTINUE:
            heading_diff = float(terminal_metrics.get('heading_error', 0.0))
            front_overlap = float(terminal_metrics.get('front_overlap', 0.0))
            if heading_diff < float(np.deg2rad(15)) and front_overlap > 0.7:
        ...
        collision_part = None
        unique_collision_parts = sorted(set(collision_parts))
        if len(unique_collision_parts) == 1:
            collision_part = unique_collision_parts[0]
        elif len(unique_collision_parts) > 1:
            collision_part = 'front_and_rear'
